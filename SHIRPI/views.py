...
def browse(request, restaurant_name = None, restaurant_address = None, api_flag = None):
	...
	try:
		results = Restaurant.objects.filter(name__icontains=restaurant_name, address_searchable__icontains=restaurant_address, health_report_status__gte=lower_limit, health_report_status__lt=upper_limit).order_by(type + order, '-health_report_status')
